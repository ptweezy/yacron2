...
| --- | --- | --- | --- |
| `-c`, `--config` | path (file or directory) | `/etc/yacron2.d` | Configuration file, or a directory containing configuration files. When a directory, every `*.yml`/`*.yaml` file in it is loaded (entries whose name starts with `_` or `.` are skipped). See [Includes, Defaults, and Multi-File Config](Includes-and-Defaults). |
...
The default is `/etc/yacron2.d`. The special case is triggered by the condition
`args.config == "/etc/yacron2.d" and not os.path.exists(args.config)`: if the
config argument equals the default path and that path does not exist, yacron2
prints the following to stderr, prints the usage help, and exits `1`:
...
it fires both when `-c` is omitted and when you pass `-c /etc/yacron2.d`
explicitly. For any other non-existent path passed with `-c`, you instead get
the generic configuration-error path (a logged `Configuration error: ...` and
exit `1`).
...
`/etc/yacron2.d` and that path is absent exits `1` with the not-found message
rather than the `Configuration error: ...` message.
...
3. Installs handlers for `SIGINT` and `SIGTERM`.
...
| `1` | Configuration error (parse/schema/validation failure or unreadable config); or the default `-c` path `/etc/yacron2.d` does not exist and no `-c` was given. |
...
see [Production and Container Deployment](Production-Deployment).
